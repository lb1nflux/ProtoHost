...
后端通过 [backend/src/main/resources/application.yml](backend/src/main/resources/application.yml) 中的占位符结合环境变量完成配置，需重点确认：
...
- `app.cors.allowed-origins`：允许访问后端接口的前端来源列表，多个值用英文逗号分隔
...
当前仓库中未包含独立 License 文件；如果计划公开分发，建议补充明确的许可证与版权说明。
